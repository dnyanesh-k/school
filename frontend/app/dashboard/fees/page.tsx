...
function DefaulterRow({ defaulter, dueSoon = false }: { defaulter: Defaulter; dueSoon?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Call button ΓÇö opens phone dialer now; swap href to Exotel API later */}
      <a
      ...
      {loadingClasses ? (
        ...
        <>
          <ClassFilterTabs
            ...
            onSelect={(id) => { setSelectedClass(id); setFeeSearch(""); }}
          />

          <ViewToggle
            view={activeView}
            onChange={(v) => { setActiveView(v); setFeeSearch(""); }}
              ...
              {loadingOverview ? (
                ...
                <>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {summaries
                      .filter((s) => s.student_name.toLowerCase().includes(feeSearch.toLowerCase()))
                      .map((summary) => (
                        <StudentFeeRow
